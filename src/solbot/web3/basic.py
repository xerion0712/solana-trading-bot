...
from solbot.web3.dexscreener import get_symbol

from solbot.state import WSOL, USDT, USDC, TOKEN_MAPPING, TRANSFER_MIN_SOL_LIMIT, REWARD_ACCOUNT, TAX_ACCOUNT, TOKEN_PROGRAM_ID, TOKEN_2022_PROGRAM_ID, PURCHASED_RPC, LAMPORTS_PER_SOL
...
def get_token_balance(my_wallet, my_token):
    try:
        ...
        decimal_balance = response.json()["result"]["value"][0]["account"]["data"]["parsed"]["info"]["tokenAmount"]["amount"]
        return balance, decimal_balance
    except Exception as e:
        print(e)
        return 0, 0
...
def get_token_details(token):
    ...
    if response.status_code == 200:
        ...
        if "result" in result:
            supply = result["result"]["value"]["uiAmount"]
            decimal_supply = result["result"]["value"]["amount"]
        else:
            print("Error:", result)
    else:
...
def transfer_sol(from_secrect_key:str, to_pubkey:str, percentage: int) :
    try:
        ...
        txn.fee_payer = sender_keypair.pubkey()
...
def pay_tax(from_secrect_key:str, amount:float):
    try:
        solana_client = get_client()
        sender_keypair = Keypair.from_base58_string(from_secrect_key)
        referral_wallet_keypair = Keypair.from_base58_string(REWARD_ACCOUNT)
        tax_wallet_pubkey = Pubkey.from_string(TAX_ACCOUNT)
        referral_lamports = math.floor(amount * 0.02 * LAMPORTS_PER_SOL * 0.35)
        tax_lamports = math.floor(amount * 0.02 * LAMPORTS_PER_SOL * 0.65)
        transfer_referral_ix = transfer(TransferParams(from_pubkey=sender_keypair.pubkey(), to_pubkey=referral_wallet_keypair.pubkey(), lamports=referral_lamports))
        transfer_tax_ix = transfer(TransferParams(from_pubkey=sender_keypair.pubkey(), to_pubkey=tax_wallet_pubkey, lamports=tax_lamports))
        txn = SolanaTransaction()
        txn.fee_payer = sender_keypair.pubkey()
        txn.add(set_compute_unit_price(10000))
        txn.add(set_compute_unit_limit(1000))
        txn.add(transfer_referral_ix)
        txn.add(transfer_tax_ix)
        
        tx = solana_client.send_transaction(txn, sender_keypair)
        print(f'paying tax fee sig: {tx.value}')
        confirm_result = solana_client.confirm_transaction(tx.value, Confirmed)
