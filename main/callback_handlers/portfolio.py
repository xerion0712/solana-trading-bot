...
from web3.basic import  get_token_authority, get_token_balance, get_token_portfolio, get_client, get_token_information, format_number
...
async def portfolio(update: Update, context: CallbackContext, text:str):
    ...
    try:
        ...
        print(e)
        context.bot.send_message(chat_id, 'Some error occurs')
...
def handle_portfolio(update: Update, context: CallbackContext):
    chat_id = update.effective_chat.id
    message_id = update.callback_query.message.message_id
    hex_priv_key = get_priv_key(chat_id)
