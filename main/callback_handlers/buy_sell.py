...
from web3.basic import get_wallet_pubkey, get_token_balance, get_token_metadata, get_client
from web3.swap import handle_sell_token, handle_buy_token
from postgre_sql import get_priv_key

from state import user_states, user_logs, LAMPORTS_PER_SOL, BUY_SELL_TOKEN_ADDRESS
...
def buy_option(bot, chat_id, message_id, token_address):
    ...
    try:
        keyboard = [
            ...
            # [InlineKeyboardButton("RAPID FIRE", callback_data=f"BATCHBUY_{token_address}"), InlineKeyboardButton("CANCEL", callback_data=f"EXIT_FUN_{token_address}")],
        ]
...
def sell_option(bot, chat_id, message_id, token_address):
    try:
        ...
        token_balance = get_token_balance(str(pubkey),str(token_address))
        if token_balance:
            ...
            keyboard = [
                ...
                # [InlineKeyboardButton("Enter Number of tokens", callback_data=f"INPUTSELLTOKEN_{token_address}")],
                [InlineKeyboardButton("CANCEL", callback_data=f"EXIT_FUN_{token_address}")]
