...
from web3.basic import get_token_information, get_token_authority, get_token_balance, get_client, format_number
from postgre_sql import get_priv_key

from state import watch_list, WSOL, LAMPORTS_PER_SOL
...
def handle_watch_list(update: Update, context: CallbackContext) -> None:
    ...
    keyboard = []
    message = 'Watch list'
    if len(watch_list.get(chat_id,[])) == 0:
        message += ' empty'
    else:
        for token in watch_list[chat_id]:
            keyboard.append([InlineKeyboardButton(f"{token}", callback_data=f"TOKEN_WATCH_{token}")])
